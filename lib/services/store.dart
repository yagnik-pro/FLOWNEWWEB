...
class AppStore extends ChangeNotifier {
  ...
  Future<void> _fetchDetails(Account a) async {
    try {
      final d = await WebSession.apiPost(a.cookies, '/api/container/supplier/getSupplierDetails');
      final id = MeeshoApi.digInto(d, const ['supplier_id', 'supplierId', 'id']);
      final nm = MeeshoApi.digInto(d, const ['name', 'supplier_name', 'business_name', 'shop_name']);
      var changed = false;
      if (id != null && id.isNotEmpty && a.supplierId.isEmpty) { a.supplierId = id; changed = true; }
      if (nm != null && nm.isNotEmpty && a.autoName) { a.name = nm; changed = true; }
